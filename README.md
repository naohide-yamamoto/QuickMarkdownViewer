...
Stillic is a native Markdown viewer for opening Markdown files as clean, rendered documents on macOS.

It is designed to feel like using Mac's Preview app for viewing Markdown documents, without editing tools or setup.

Stillic is free and open-source. If you find it useful, [optional contributions are greatly appreciated](https://ko-fi.com/naohideyamamoto) and help cover ongoing maintenance costs. Contributions are voluntary and do not change Stillic’s functionality.
...
- [What Stillic Is For](#what-stillic-is-for)
...
- [What Stillic Is Not For](#what-stillic-is-not-for)
- [Support](#support)
...
2. Download the latest app ZIP (`Stillic-v<version>-macOS.zip`).
3. Open the ZIP and drag `Stillic.app` into `Applications`.
4. Open the app once.
5. Open a Markdown file by either:
   - double-clicking a supported Markdown file associated with Stillic
   - right-clicking a Markdown file and choosing **Open With > Stillic**
...
If you select Stillic as the default Markdown viewer in Settings (`⌘,`), macOS applies that choice to its grouped Markdown types (currently `.md` and `.markdown`).
...
Stillic is intended for standard Markdown document rendering. It is not designed to reproduce non-standard or modified Markdown formats. For example, the links to section headings used in the table of contents above, which are supported by GitHub's Markdown processor, do not work on Stillic.

## Set as the Default App

You can set Stillic as the default Markdown viewer from Settings:
...
- View raw source in the app selected in `Settings > General > View source with`.
  - If the selected app becomes unavailable, Stillic falls back to the system default text editor.
- `Check for Updates…` in the app menu, with optional automatic checking.
  - Automatic checking is off by default.
  - When enabled, Stillic contacts GitHub only to check whether a newer version is available.
...
Stillic source code is licensed under the Apache License, Version 2.0.
...
The Stillic name, app icon, and related branding are not licensed under Apache-2.0. See [TRADEMARKS.md](TRADEMARKS.md).

## Official Builds and Forks

Stillic is open-source software. You are welcome to read the code, build it, fork it, and modify it under the terms of the Apache License, Version 2.0.
...
If you obtain Stillic from another source, or from a fork, treat that build as unofficial unless it is clearly identified as an official release from this repository and carries the expected signing/notarisation status.

If you redistribute a modified build, please rename the app and replace the icon and related branding unless you have explicit permission to use the official Stillic branding.

## No Warranty

Stillic is provided under the Apache License, Version 2.0 on an 'AS IS' basis, without warranties or conditions of any kind.
...
- Current practical behaviour: `Help > Stillic Help` opens bundled in-app Help content directly instead of relying on Help Viewer page loading.
...
- On some macOS builds, the app menu `Settings…` item may still display the generic gear icon even when Stillic requests `gearshape`.
- Default-app association UI may not live-refresh across open windows. For example, changing the Markdown default app in Stillic Settings may not immediately update an already-open Finder `Get Info` panel, and changing it in Finder may not immediately update an already-open Stillic Settings pane.
...
For other project-related enquiries, please email [hello@stillic.app](mailto:hello@stillic.app).
