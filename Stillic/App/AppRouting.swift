...
extension Notification.Name {
    static let stillicFindCommand = Notification.Name("Stillic.FindCommand")

    /// Notification posted when the app-level Zoom commands should act on a window.
    static let stillicZoomCommand = Notification.Name("Stillic.ZoomCommand")

    /// Notification posted when document actions should act on a window.
    static let stillicDocumentCommand = Notification.Name("Stillic.DocumentCommand")

    /// Notification posted when a document window publishes Find state updates.
    static let stillicFindStateDidChange = Notification.Name("Stillic.FindStateDidChange")
}

/// Supported in-document Find actions for Stillic windows.
///
...
enum StillicFindCommand: String {
...
/// Keys used in `stillicFindCommand` notification payloads.
enum StillicFindCommandUserInfoKey: String {
    /// The `StillicFindCommand.rawValue` to execute.
    case command
...
/// Supported in-document zoom actions for Stillic windows.
///
...
enum StillicZoomCommand: String {
...
/// Keys used in `stillicZoomCommand` notification payloads.
enum StillicZoomCommandUserInfoKey: String {
    /// The `StillicZoomCommand.rawValue` to execute.
    case command
...
/// Supported document actions for Stillic windows.
///
...
enum StillicDocumentCommand: String {
...
/// Keys used in `stillicDocumentCommand` notification payloads.
enum StillicDocumentCommandUserInfoKey: String {
    /// The `StillicDocumentCommand.rawValue` to execute.
    case command
...
final class StillicAppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Stillic reliable when users double-click Markdown files.
    ...
    ///
    /// Stillic controls fresh-window behaviour explicitly at launch, so
    ...
    /// tiny utility behaviour expected from Stillic.
    ...
    ///
    /// Stillic controls its own empty/document window flow, so allowing this
...
final class AppRouting: ObservableObject {
    ...
    private enum UnsupportedOpenReason {
        ...
        /// The file extension is not one Stillic accepts.
    ...
    private let defaultViewerOpenPanelLastDirectoryDefaultsKey =
        "Stillic.DefaultViewerOpenPanel.LastDirectory.v1"
    ...
    private let viewSourceOpenPanelLastDirectoryDefaultsKey =
        "Stillic.ViewSourceAppOpenPanel.LastDirectory.v1"

    /// Sentinel picker ID representing "follow system default text editor".
    private let viewSourceSystemDefaultOptionID = "__stillic.viewSource.systemDefault__"
    ...
    private let latestReleaseAPIURL = URL(
        string: "https://api.github.com/repos/naohide-yamamoto/stillic/releases/latest"
    )
    ...
    private func showEmptyWindow() {
        ...
        // default when a new Stillic window appears.
    ...
    /// immediate guidance on what Stillic can open.
    private func presentUnsupportedFileWarning(for fileURL: URL, reason: UnsupportedOpenReason) {
        ...
        case .unsupportedMarkdownVariant:
            switch ext {
            case "rmd":
                alert.messageText = "Stillic does not support .rmd files."
                alert.informativeText =
                    "Stillic is a plain Markdown viewer. R Markdown files usually require knitr/pandoc rendering, which Stillic does not run in v1."

            case "qmd":
                alert.messageText = "Stillic does not support .qmd files."
                alert.informativeText =
                    "Stillic is a plain Markdown viewer. Quarto files usually require Quarto rendering features that are outside Stillic v1."

            default:
                alert.messageText = "Stillic cannot open .\(ext) files."
                alert.informativeText =
                    "Stillic only accepts Markdown documents with these extensions: \(acceptedList)."
            }

        case .unsupportedFileType:
            if ext.isEmpty {
                alert.messageText = "Stillic cannot open this file type."
                alert.informativeText =
                    "Stillic only accepts Markdown documents with these extensions: \(acceptedList)."
            } else {
                alert.messageText = "Stillic cannot open .\(ext) files."
    ...
    private func presentMissingHelpWarning() {
        ...
        alert.informativeText =
            "Stillic could not find a registered Apple Help Book."
        alert.addButton(withTitle: "OK")
        alert.runModal()
    }
    ...
    private func dispatchFindCommandToActiveWindow(_ command: StillicFindCommand) {
        guard let targetWindow = activeDocumentWindow() else {
            NSSound.beep()
            return
        }
        ...
        NotificationCenter.default.post(
            name: .stillicFindCommand,
            object: targetWindow,
            userInfo: [
                StillicFindCommandUserInfoKey.command.rawValue: command.rawValue
            ]
        )
    }
    ...
    /// time.
    private func dispatchZoomCommandToActiveWindow(_ command: StillicZoomCommand) {
        guard let targetWindow = activeDocumentWindow() else {
            NSSound.beep()
            return
        }
        ...
        NotificationCenter.default.post(
            name: .stillicZoomCommand,
            object: targetWindow,
            userInfo: [
                StillicZoomCommandUserInfoKey.command.rawValue: command.rawValue
            ]
        )
    }
    ...
    private func dispatchDocumentCommandToActiveWindow(_ command: StillicDocumentCommand) {
        guard let targetWindow = activeDocumentWindow() else {
            NSSound.beep()
            return
        }
        ...
        NotificationCenter.default.post(
            name: .stillicDocumentCommand,
            object: targetWindow,
            userInfo: [
                StillicDocumentCommandUserInfoKey.command.rawValue: command.rawValue
            ]
        )
    }
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    if trigger == .manual {
                        ...
                        alert.informativeText =
                            "Stillic could not reach GitHub release metadata.\n\n\(error.localizedDescription)"
                        alert.addButton(withTitle: "OK")
                        alert.runModal()
                    } else {
    ...
    private func fetchLatestRelease(from url: URL) async throws -> LatestGitHubRelease {
        ...
        request.setValue("Stillic", forHTTPHeaderField: "User-Agent")
    ...
    private func presentUpdateCheckResult(_ result: UpdateCheckResult, trigger: UpdateCheckTrigger) {
        ...
        case .upToDate(let installedVersion):
            ...
            let alert = NSAlert()
            alert.alertStyle = .informational
            alert.messageText = "Stillic is up to date."
    ...
    private func showDefaultMarkdownViewerAssociationFailureAlert() {
        ...
        alert.informativeText =
            "Stillic could not update macOS file association settings for Markdown files."
        alert.addButton(withTitle: "OK")
        alert.runModal()
    }
    ...
    private func showViewSourceAppSelectionFailureAlert() {
        ...
        alert.informativeText =
            "Stillic could not resolve the selected application."
        alert.addButton(withTitle: "OK")
        alert.runModal()
    }
    ...
    /// currently open source file for each Stillic document window.
...
private final class DocumentWindowController: NSWindowController, NSWindowDelegate {
    ...
    private enum ToolbarItemIdentifier {
        static let toolbar = NSToolbar.Identifier("Stillic.MainToolbar")

        static let open = NSToolbarItem.Identifier("Stillic.Toolbar.Open")
        static let zoomLegacy = NSToolbarItem.Identifier("Stillic.Toolbar.ZoomLegacy")
        static let appearance = NSToolbarItem.Identifier("Stillic.Toolbar.Appearance")
        static let printExportGroup = NSToolbarItem.Identifier("Stillic.Toolbar.PrintExportGroup")
        static let search = NSToolbarItem.Identifier("Stillic.Toolbar.Search")

        static let share = NSToolbarItem.Identifier("Stillic.Toolbar.Share")
        static let viewSource = NSToolbarItem.Identifier("Stillic.Toolbar.ViewSource")
        static let zoomToFit = NSToolbarItem.Identifier("Stillic.Toolbar.ZoomToFit")
        static let actualSize = NSToolbarItem.Identifier("Stillic.Toolbar.ActualSize")
        static let zoomOutIn = NSToolbarItem.Identifier("Stillic.Toolbar.ZoomOutIn")
        static let print = NSToolbarItem.Identifier("Stillic.Toolbar.Print")
        static let exportPDF = NSToolbarItem.Identifier("Stillic.Toolbar.ExportPDF")
    }
    ...
    ///
    /// This keeps Stillic in sync with external edits while staying
    ...
    /// Stillic. We reuse the same window, but refresh content so
    ...
    private func installFindStateObserver(for window: NSWindow) {
        findStateObserver = NotificationCenter.default.addObserver(
            forName: .stillicFindStateDidChange,
            object: window,
            queue: .main
        ) { [weak self] notification in
            let query =
                notification.userInfo?[StillicFindCommandUserInfoKey.query.rawValue] as? String ?? ""
            let caseSensitive =
                notification.userInfo?[StillicFindCommandUserInfoKey.isCaseSensitive.rawValue] as? Bool ?? false
            let shouldFocusFindUI =
                notification.userInfo?[StillicFindCommandUserInfoKey.shouldFocusFindUI.rawValue] as? Bool ?? false
    ...
    private func dispatchFindQueryUpdate(
        ...
        NotificationCenter.default.post(
            ...
            object: hostWindow,
            userInfo: [
                StillicFindCommandUserInfoKey.command.rawValue: StillicFindCommand.setFindQuery.rawValue,
                StillicFindCommandUserInfoKey.query.rawValue: currentFindQuery,
                StillicFindCommandUserInfoKey.shouldRunSearch.rawValue: shouldRunSearch,
                StillicFindCommandUserInfoKey.shouldBeepOnNoMatch.rawValue: shouldBeepOnNoMatch
            ]
        )
    }
    ...
    private func dispatchFindCaseSensitivityUpdate() {
        ...
        NotificationCenter.default.post(
            ...
            object: hostWindow,
            userInfo: [
                StillicFindCommandUserInfoKey.command.rawValue:
                    StillicFindCommand.setFindCaseSensitivity.rawValue,
                StillicFindCommandUserInfoKey.isCaseSensitive.rawValue: isFindCaseSensitive
            ]
        )
    }
    ...
    @objc private func zoomInFromToolbar(_ sender: Any?) {
        ...
        NotificationCenter.default.post(
            ...
            object: hostWindow,
            userInfo: [
                StillicZoomCommandUserInfoKey.command.rawValue: StillicZoomCommand.zoomIn.rawValue
            ]
        )
    }
    ...
    @objc private func zoomOutFromToolbar(_ sender: Any?) {
        ...
        NotificationCenter.default.post(
            ...
            object: hostWindow,
            userInfo: [
                StillicZoomCommandUserInfoKey.command.rawValue: StillicZoomCommand.zoomOut.rawValue
            ]
        )
    }
    ...
    @objc private func actualSizeFromToolbar(_ sender: Any?) {
        ...
        NotificationCenter.default.post(
            ...
            object: hostWindow,
            userInfo: [
                StillicZoomCommandUserInfoKey.command.rawValue:
                    StillicZoomCommand.resetToActualSize.rawValue
            ]
        )
    }
    ...
    @objc private func zoomToFitFromToolbar(_ sender: Any?) {
        ...
        NotificationCenter.default.post(
            ...
            object: hostWindow,
            userInfo: [
                StillicZoomCommandUserInfoKey.command.rawValue: StillicZoomCommand.zoomToFit.rawValue
            ]
        )
    }
    ...
    @objc private func printFromToolbar(_ sender: Any?) {
        ...
        NotificationCenter.default.post(
            ...
            object: hostWindow,
            userInfo: [
                StillicDocumentCommandUserInfoKey.command.rawValue:
                    StillicDocumentCommand.printRenderedDocument.rawValue
            ]
        )
    }
    ...
    @objc private func exportPDFFromToolbar(_ sender: Any?) {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                StillicDocumentCommandUserInfoKey.command.rawValue:
                    StillicDocumentCommand.exportRenderedPDF.rawValue
            ]
        )
    }
    ...
    @objc private func viewSourceFromToolbar(_ sender: Any?) {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                StillicDocumentCommandUserInfoKey.command.rawValue:
                    StillicDocumentCommand.viewSourceExternally.rawValue
            ]
        )
    }
    ...
    func control(_ control: NSControl, textView: NSTextView, doCommandBy commandSelector: Selector) -> Bool {
        ...
        if let hostWindow = window {
            NotificationCenter.default.post(
                ...
                userInfo: [
                    StillicFindCommandUserInfoKey.command.rawValue:
                        StillicFindCommand.findNext.rawValue
                ]
            )
        }
        return true
