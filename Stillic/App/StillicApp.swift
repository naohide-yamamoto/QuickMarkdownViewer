...
enum AppPreferenceKey {
    ...
    static let defaultWindowWidth = "stillic.defaultWindowWidth"

    /// Default height (points) for newly opened windows.
    static let defaultWindowHeight = "stillic.defaultWindowHeight"
    ...
    static let windowBackgroundVisibility = "stillic.windowBackgroundVisibility"

    /// Hex colour string (`#RRGGBB`) used for light-mode window background.
    static let windowBackgroundColorLightHex = "stillic.windowBackgroundColorLightHex"

    /// Hex colour string (`#RRGGBB`) used for dark-mode window background.
    static let windowBackgroundColorDarkHex = "stillic.windowBackgroundColorDarkHex"

    /// True when automatic release-metadata checks are enabled.
    static let automaticUpdateCheckEnabled = "stillic.automaticUpdateCheckEnabled"

    /// True when fenced-code syntax highlighting is enabled.
    static let syntaxHighlightingEnabled = "stillic.syntaxHighlightingEnabled"

    /// Selected syntax-highlighting theme family.
    static let syntaxHighlightingTheme = "stillic.syntaxHighlightingTheme"

    /// Selected document typeface for rendered Markdown typography.
    static let documentTypeface = "stillic.documentTypeface"

    /// Selected document density variant.
    static let documentDensity = "stillic.documentDensity"

    /// Selected toolbar button size.
    static let toolbarButtonSize = "stillic.toolbarButtonSize"
...
/// Main application entry point for Stillic.
///
...
struct StillicApp: App {
    ...
    @NSApplicationDelegateAdaptor(StillicAppDelegate.self) private var appDelegate
    ...
    var body: some Scene {
        Settings {
            StillicSettingsView(routing: routing)
        }
        .commands {
...
private struct StillicSettingsView: View {
    /// Sentinel picker ID used for the bottom "Select…" action row.
    private static let selectDefaultViewerOptionID = "__stillic.selectDefaultViewer__"

    /// Sentinel picker ID used for View Source app "Select…" action row.
    private static let selectViewSourceAppOptionID = "__stillic.selectViewSourceApp__"
    ...
    ///
    /// Sized to fit "Stillic" in full while remaining compact.
    ...
    private var markdownViewerPopupOptions: [GeneralPaneAppPicker.Option] {
        ...
        if !markdownViewerOptions.isEmpty {
            result.append(
                GeneralPaneAppPicker.Option(
                    id: "__stillic.separator.defaultViewer__",
                    title: "",
                    icon: nil,
                    isSeparator: true
                )
    ...
    private var viewSourceAppPopupOptions: [GeneralPaneAppPicker.Option] {
        ...
        if !viewSourceAppOptions.isEmpty {
            result.append(
                GeneralPaneAppPicker.Option(
                    id: "__stillic.separator.viewSource__",
                    title: "",
                    icon: nil,
                    isSeparator: true
                )
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    generalPaneRow("Updates:") {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text("When enabled, Stillic automatically contacts GitHub to check for updates.")
...
private struct SyntaxHighlightPreviewWebView: NSViewRepresentable {
    ...
    private func makeHTML(configuration: SyntaxHighlightPreviewConfiguration) -> String {
        ...
        let previewTypefaceClass = "stillic-typeface-\(configuration.typeface.rawValue)"
        let previewDensityClass = "stillic-density-\(configuration.density.rawValue)"
