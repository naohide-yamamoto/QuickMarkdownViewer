...
final class MarkdownWebViewSearchBridge: ObservableObject {
    ...
    func find(
        ...
        (() => {
            ...
            return window.__stillicFindController.run(
                root,
                \(escapedQuery),
                \(backwardsLiteral),
    ...
    func clearFindHighlights() -> Bool {
        ...
        (() => {
            ...
            \(findControllerBootstrapJavaScript())
            return window.__stillicFindController.clear(root);
        })();
        """
    ...
    /// JavaScript bootstrap for Stillic's in-page find/highlight controller.
    ///
    ...
    private func findControllerBootstrapJavaScript() -> String {
        """
        if (!window.__stillicFindController) {
            window.__stillicFindController = (() => {
                const hitClass = "stillic-find-hit";
                const activeClass = "stillic-find-hit-active";
    ...
    ///
    /// This explicitly enables `.stillic-print-output` mode so exported output is
    ...
    private func temporaryOutputHTML(from snapshot: TemporaryOutputSnapshot) -> String {
        ...
        let outputBodyClass = inheritedBodyClasses.isEmpty
            ? "stillic-print-output"
            : "\(inheritedBodyClasses) stillic-print-output"
    ...
    /// Normalises an arbitrary zoom value into Stillic's supported range.
...
struct MarkdownWebView: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        /// Callback for opening local Markdown links in Stillic.
        ...
        ///
        /// This is the standard macOS smart-zoom gesture, mapped to Stillic's
        ...
        /// Intercepts clicked links and applies Stillic routing policy.
        func webView(
            _ webView: WKWebView,
        ...
        func applyWindowBackgroundPreferencesIfNeeded(on webView: WKWebView, force: Bool = false) {
            ...
            (() => {
                ...
                root.style.setProperty(
                    '--stillic-background-scale',
                    String(Math.max(0, Math.min(1, visibility)))
                );
                root.style.setProperty('--stillic-background-custom-color-light', \(lightHexLiteral));
                root.style.setProperty('--stillic-background-custom-color-dark', \(darkHexLiteral));
            })();
            """
        ...
        func applySyntaxHighlightingPreferenceIfNeeded(on webView: WKWebView, force: Bool = false) {
            ...
            let script = """
            (() => {
                const renderer = window.StillicRenderer;
