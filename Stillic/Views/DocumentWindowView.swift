...
struct DocumentWindowView: View {
    ...
    /// Callback to open a file URL in Stillic.
    ...
    ///
    /// Default remains `false` so Stillic keeps today's case-insensitive
    ...
    var body: some View {
        ...
        .onReceive(
            NotificationCenter.default.publisher(for: .stillicFindCommand),
            perform: handleFindNotification(_:)
        )
        .onReceive(
            NotificationCenter.default.publisher(for: .stillicZoomCommand),
            perform: handleZoomNotification(_:)
        )
        .onReceive(
            NotificationCenter.default.publisher(for: .stillicDocumentCommand),
    ...
    private func handleFindNotification(_ notification: Notification) {
        guard let commandRaw = notification.userInfo?[StillicFindCommandUserInfoKey.command.rawValue] as? String,
              let command = StillicFindCommand(rawValue: commandRaw),
    ...
    private func handleZoomNotification(_ notification: Notification) {
        guard let commandRaw = notification.userInfo?[StillicZoomCommandUserInfoKey.command.rawValue] as? String,
              let command = StillicZoomCommand(rawValue: commandRaw),
              shouldHandleNotification(notification) else {
            return
        }
    ...
    private func handleDocumentNotification(_ notification: Notification) {
        guard let commandRaw =
                notification.userInfo?[StillicDocumentCommandUserInfoKey.command.rawValue] as? String,
              let command = StillicDocumentCommand(rawValue: commandRaw),
              shouldHandleNotification(notification) else {
            return
        }
    ...
    private func shouldHandleNotification(_ notification: Notification) -> Bool {
        ...
        if let targetWindowNumber =
            notification.userInfo?[StillicDocumentCommandUserInfoKey.targetWindowNumber.rawValue] as? Int {
            return targetWindowNumber == windowNumber
        }

        if let targetWindowNumber =
            notification.userInfo?[StillicFindCommandUserInfoKey.targetWindowNumber.rawValue] as? Int {
            return targetWindowNumber == windowNumber
        }

        if let targetWindowNumber =
            notification.userInfo?[StillicZoomCommandUserInfoKey.targetWindowNumber.rawValue] as? Int {
            return targetWindowNumber == windowNumber
        }
    ...
    private func handleFindCommand(_ command: StillicFindCommand, userInfo: [AnyHashable: Any]?) {
        ...
        case .setFindQuery:
            let updatedQuery =
                userInfo?[StillicFindCommandUserInfoKey.query.rawValue] as? String ?? ""
            let updatedCaseSensitivity =
                userInfo?[StillicFindCommandUserInfoKey.isCaseSensitive.rawValue] as? Bool
            let shouldRunSearch =
                userInfo?[StillicFindCommandUserInfoKey.shouldRunSearch.rawValue] as? Bool ?? true
            let shouldBeep =
                userInfo?[StillicFindCommandUserInfoKey.shouldBeepOnNoMatch.rawValue] as? Bool ?? false
        ...
        case .setFindCaseSensitivity:
            guard let isCaseSensitive =
                userInfo?[StillicFindCommandUserInfoKey.isCaseSensitive.rawValue] as? Bool else {
                return
            }
    ...
    private func handleZoomCommand(_ command: StillicZoomCommand) {
        guard canUseDocumentControls else {
            NSSound.beep()
            return
        }
    ...
    private func handleDocumentCommand(_ command: StillicDocumentCommand) {
        guard canUseDocumentControls else {
            NSSound.beep()
            return
        }
    ...
    private func exportRenderedPDF() {
        ...
        if let sourceURL = documentState.document?.fileURL {
            ...
            suggestedFilename = baseName.isEmpty ? "Stillic Export.pdf" : "\(baseName).pdf"
        } else {
            suggestedFilename = "Stillic Export.pdf"
        }
    ...
    private func publishFindStateChange() {
        ...
        NotificationCenter.default.post(
            name: .stillicFindStateDidChange,
            object: hostWindow,
            userInfo: [
                StillicFindCommandUserInfoKey.query.rawValue: findQuery,
                StillicFindCommandUserInfoKey.isCaseSensitive.rawValue: isCaseSensitiveSearch,
                StillicFindCommandUserInfoKey.shouldFocusFindUI.rawValue: shouldFocusFindUI
            ]
        )
    }
...
///
/// Using `NSSearchField` gives Stillic the expected AppKit behaviour:
...
private struct AppearanceSegmentedControl: NSViewRepresentable {
    ...
    /// state remains visible without drawing too much attention in Stillic's
...
/// document window, even when multiple Stillic windows are open.
